...
FLASH_EraseInitTypeDef My_EraseInitStruct;
uint32_t My_Address = 0, My_PAGEError = 0;
uint32_t MY_FirstPage = 0, MY_NbOfPages = 0, MY_BankNumber = 0;
void	Flash_Erase(uint32_t Address,uint32_t NumToWrite)	
{
  ...
  MY_FirstPage = GetPage(Address);
  /* Get the number of pages to erase from 1st page */
  MY_NbOfPages = GetPage(Address+NumToWrite) - MY_FirstPage + 1;
  /* Get the bank */
  MY_BankNumber = GetBank(Address);
  /* Fill EraseInit structure*/
  My_EraseInitStruct.TypeErase   = FLASH_TYPEERASE_PAGES;
  My_EraseInitStruct.Banks       = MY_BankNumber;
  My_EraseInitStruct.Page        = MY_FirstPage;
  My_EraseInitStruct.NbPages     = MY_NbOfPages;

  /* Note: If an erase operation in Flash memory also concerns data in the data or instruction cache,
     you have to make sure that these data are rewritten before they are accessed during code
     execution. If this cannot be done safely, it is recommended to flush the caches by setting the
     DCRST and ICRST bits in the FLASH_CR register. */
  if (HAL_FLASHEx_Erase(&My_EraseInitStruct, &My_PAGEError) != HAL_OK)
	{}
}
...
//	STMFLASH_Read(EEPROM_START,(uint32_t*)&config,sizeof(config.vaildsign));
	...
	{	
		...
		{
			if(config.Iap_datalength == 0xffffffff)
			{
				config.Iap_flag = 0;
			}
		}
